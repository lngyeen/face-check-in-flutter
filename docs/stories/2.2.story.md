...
3. Frames are sent to the backend WebSocket as a JSON object containing `type: 'processFrame'` and `image: <base64_string>`.
...
- [x] Task 4: Frame Streaming Service
  ...
  - [x] Implement frame transmission with a JSON payload (`{ "type": "processFrame", "image": "..." }`)
...
- [x] Task 8: Face Detection Data Management
  - [x] Create face detection data models
  - [x] Implement face detection result storage in BLoC state
...
#### WebSocket Payload for Frame Processing
The message sent to the WebSocket for each frame must be a JSON string with the following structure:
```json
{
  "type": "processFrame",
